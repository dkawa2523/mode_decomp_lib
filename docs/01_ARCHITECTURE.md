...
8. inference: 予測係数 → field_hat
9. evaluate: coeff誤差 + field誤差 + 解釈性補助（主に decomposition/inference で実施）
...
- 拡張の起点は `configs/*/*.yaml` と `task=pipeline` の sweep 設定。
...
## Pipeline sweep
- `task=pipeline` の decompose_list / coeff_post_list / model_list で sweep を実行する。
