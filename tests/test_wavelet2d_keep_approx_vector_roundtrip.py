...
def test_wavelet2d_keep_approx_roundtrip_vector_stacks_channels() -> None:
    rng = np.random.default_rng(0)
    field = rng.normal(size=(32, 32, 2)).astype(np.float32)
    domain = build_domain_spec(
    ...
    # Reference: per-channel wavedec2, zero-out details, and reconstruct.
    ref_channels = []
    for ch in range(2):
        coeffs = pywt.wavedec2(field[..., ch], wavelet="db2", mode="symmetric", level=2)
        cA = coeffs[0]
        zeros_details = []
        ...
        ref_channels.append(ref)
    ref_stack = np.stack(ref_channels, axis=-1).astype(np.float32)

    assert np.allclose(field_hat, ref_stack, atol=1e-6)
