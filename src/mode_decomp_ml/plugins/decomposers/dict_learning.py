...
def _choose_transform_algorithm(
    ...
    *,
    ...
    if n_components is None:
        return transform_algorithm
    if n_components <= n_samples and n_components <= n_features:
        return transform_algorithm
    if warn_on_change:
        _LOGGER.warning(
            "dict_learning: switching transform_algorithm from 'omp' to 'lasso_cd' "
            "because n_components exceeds sample/feature count."
        )
    return "lasso_cd"
...
class DictLearningDecomposer(BaseDecomposer):
    ...
    def __init__(self, *, cfg: Mapping[str, Any]) -> None:
        ...
        self._warn_on_omp_fallback = bool(cfg_get(cfg, "warn_on_omp_fallback", True))
        ...
        self._transform_algorithm_effective: str | None = None
    ...
    def fit(
        ...
        for ch in range(channels):
            ...
            transform_algorithm = _choose_transform_algorithm(
                self._transform_algorithm,
                n_components=self._n_components,
                n_samples=snapshots.shape[0],
                n_features=snapshots.shape[1],
                warn_on_change=self._warn_on_omp_fallback,
            )
        ...
        self._models = models
        self._transform_algorithm_effective = transform_algorithm
        self._mask = mask
    ...
    def transform(
        ...
        self._coeff_meta = {
            ...
            "mean_centered": False,
            "fit_algorithm": self._fit_algorithm,
            "transform_algorithm": self._transform_algorithm_effective,
            "transform_n_nonzero_coefs": self._transform_n_nonzero,
            "transform_alpha": self._transform_alpha,
            "alpha": self._alpha,
            "positive_code": self._positive_code,
